...
class get_model(nn.Module):
    def __init__(self, args):
        ...
        self.featureFusion = FeatureFusionNet()
    ...
    def forward(self, x, Lr_Info):
       ...
       # out = FormOutput(out) + x_upscale
        out_new = self.featureFusion(out) # sen_add: 增加3D卷积，实现多角度特征融合

        my_1, my_2, my_x_dim, my_y_dim = x_upscale.shape

        out = out_new + x_upscale[:,:,my_x_dim//5*2:my_x_dim//5*3,my_y_dim//5*2:my_y_dim//5*3] #与最中间的做累加
        return out
