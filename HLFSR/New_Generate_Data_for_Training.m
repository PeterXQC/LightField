...
downRatio = 1/factor;
src_data_path = 'D:\XuQichen\LightField\GTWithNoise\train';
num_datasets = 1;  % Assuming each ground truth image is treated as a separate dataset

%% Training data generation
src_datasets = dir(src_data_path);
src_datasets(1:2) = [];
num_images = length(src_datasets);

for index_image = 1:num_images
    idx_save = 0;
    name_dataset = src_datasets(index_image).name;
    name_dataset(end-3:end) = [];
    fprintf('Generating training data for Image_%s...\n', name_dataset);
    data_path = fullfile(src_data_path, name_dataset);
    data = load(data_path);
    LF = data.LF; 
    [U, V, ~, ~, ~] = size(LF);
    ...
    for h = 1 : stride : H - patchsize + 1
        for w = 1 : stride : W - patchsize + 1
            ...
            SavePath_H5 = fullfile(SavePath, [num2str(idx_save,'%06d'), '.h5']);
    ...
    fprintf([num2str(idx_save), ' training samples have been generated for Image_', name_dataset, '\n']);
end
